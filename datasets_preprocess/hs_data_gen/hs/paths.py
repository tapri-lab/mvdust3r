...
def list_scenes(base_output_dir, base_path):
    print('inside list scenes', base_output_dir, base_path)
    """
    ...
    for root, dirs, files in os.walk(base_path, followlinks=True):
        ...
        for file in files:
            ...
            if ext == ".glb":
                ...
                if relpath == ".":
                    relpath = ""
                output_dir = os.path.join(base_output_dir, relpath, name)
                data = SceneData(scene_dataset_config_file="",
                    scene = scene,
                    navmesh = navmesh,
                    output_dir = output_dir)
                folder_scenes_data.append(data)
        # print(folder_scenes_data[0], len(folder_scenes_data))
...
def list_scenes_available(base_output_dir, scenes_dataset_paths=SCENES_DATASET):
    ...
    # print(scenes_data[100], len(scenes_data))
    # input()
    # Gibson
    res = list_scenes(base_output_dir=os.path.join(base_output_dir, "gibson"),
                                base_path=scenes_dataset_paths["gibson"])
    # print(res[100], len(res))
    # input()
    # print(res)
    # print(os.path.join(base_output_dir, "gibson"), scenes_dataset_paths["gibson"])
    # input()
    scenes_data += res
