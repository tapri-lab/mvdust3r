...
def compute_overlap_ratio(pcd_world, R, t, depth_map, f, c, depth_threshold=0.05):
    ...
    """
    pcd_world = torch.from_numpy(pcd_world).cuda()
...
class MultiviewHabitatSimGenerator: # play with hfov to be the same as scannet, device should be as string like "cuda:3", specified by torchx outside.
    def __init__(self,
                scene,
                navmesh,
    ...
    def is_other_pointcloud_overlappings2(self, ref_pcd, other_pcds, ref_depth, other_depths, ref_Rt, other_Rts, f, c, is_valid, valid_fraction):
        """ Check if a viewpoint is valid and overlaps significantly with a reference one. """
        # Observation
        covs = []
        R_r, t_r = ref_Rt
        for Rt, depth, pcd in zip(other_Rts, other_depths, other_pcds):
            R, t = Rt
            overlap =  compute_overlap_ratio(ref_pcd, R, t, depth, f, c)
            overlap2 = compute_overlap_ratio(    pcd, R_r, t_r, ref_depth, f, c)
    ...
    def __getitem__(self, useless_idx):
        while 1:
            ...
            po_history = [[ref_position, ref_orientation]]
        ...
        for view_id in range(self.views_count - 1):
            ...
            for sampling_attempt in range(self.max_attempts_count):
                ...
                t.append(time.time())
                is_valid, valid_fraction, covisibility, covs = self.is_other_pointcloud_overlappings2(other_pointcloud, viewpoints_clouds, other_viewpoint_observations['depth'], all_depth, ref_Rt, all_Rt, f, c, is_valid, valid_fraction)
            ...
            if not successful_sampling:
                ...
                return self[0]
            po_history.append([position, rotation])
